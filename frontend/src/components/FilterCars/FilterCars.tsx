...
export default function FilterCars() {
  ...
  const toggleSelect = (name: string) => {
    setOpenSelect((prev) => (prev === name ? "" : name));
  };
  ...
  const fuels = [...new Set(cars.map((car) => car.fuel).filter(Boolean))];
    ...
    <div className="hero-filter-wrapper">
      <div className="main-container">
        <div className="hero-filter">
          <div
            className={`filter-select ${openSelect === "brand" ? "active" : ""}`}
          >
            <select
              value={brand}
              onMouseDown={() => toggleSelect("brand")}
              onBlur={closeSelect}
              onChange={(e) => {
          ...
          <div
            className={`filter-select ${openSelect === "model" ? "active" : ""}`}
          >
            <select
              value={model}
              disabled={!brand}
              onMouseDown={() => toggleSelect("model")}
              onBlur={closeSelect}
              onChange={(e) => {
          ...
          <div
            className={`filter-select ${openSelect === "fuel" ? "active" : ""}`}
          >
            <select
              value={fuel}
              onMouseDown={() => toggleSelect("fuel")}
              onBlur={closeSelect}
              onChange={(e) => {
          ...
          <div
            className={`filter-select ${
              openSelect === "transmission" ? "active" : ""
            }`}
          >
            <select
              value={transmission}
              onMouseDown={() => toggleSelect("transmission")}
              onBlur={closeSelect}
              onChange={(e) => {
          ...
          <button className="filter-btn" onClick={() => navigate("/catalog")}>
            <svg
              width="20"
              height="18"
              viewBox="0 0 20 18"
